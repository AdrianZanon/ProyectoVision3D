...
def generate_point_cloud(img_left, img_right, K, baseline, save_visualizations=None, show_pics=False):
    """
    Genera una nube de puntos 3D a partir de imágenes rectificadas.

    Args:
        img_left, img_right (np.ndarray): Imágenes rectificadas.
        K (np.ndarray): Matriz intrínseca.
        baseline (float): Línea base entre cámaras.
        save_visualizations (str, opcional): Directorio para guardar visualizaciones.
        show_pics (bool): Mostrar disparidad y nube de puntos (default: False).

    Returns:
        np.ndarray: Nube de puntos 3D (Nx3).
    """
    if img_left is None or img_right is None:
        print("Error: Una o ambas imágenes rectificadas no están cargadas correctamente.")
        return None

    # Validar dimensiones de las imágenes
    if img_left.shape != img_right.shape:
        print("Error: Las imágenes rectificadas tienen dimensiones diferentes.")
        return None

    # Block matching para disparidad
    window_size = 5
    max_disparity = 64
    h, w = img_left.shape
    ...
    for y in range(window_size, h - window_size):
        for x in range(window_size, w - window_size - max_disparity):
            patch_left = img_left[y - window_size:y + window_size + 1, x - window_size:x + window_size + 1]
            min_sad = float('inf')
            best_d = 0
            for d in range(max_disparity):
                # Asegurar que los índices sean válidos
                x_right_start = x - d - window_size
                x_right_end = x - d + window_size + 1
                if x_right_start < 0 or x_right_end > w:
                    continue
                patch_right = img_right[y - window_size:y + window_size + 1, x_right_start:x_right_end]
    ...
    # Guardar y mostrar mapa de disparidad
    if save_visualizations:
        os.makedirs(save_visualizations, exist_ok=True)
        disparity_viz = (disparity / max_disparity * 255).astype(np.uint8)
        cv.imwrite(os.path.join(save_visualizations, "disparity.png"), disparity_viz)

    if show_pics:
        disparity_viz = (disparity / max_disparity * 255).astype(np.uint8)
        cv.imshow('Disparity Map', disparity_viz)
        cv.waitKey(0)
        cv.destroyAllWindows()
    ...
    if save_visualizations:
        np.save(os.path.join(save_visualizations, "point_cloud.npy"), points_3d)
        # Guardar en formato PLY para visualización en herramientas externas
        with open(os.path.join(save_visualizations, "point_cloud.ply"), 'w') as f:
            f.write("ply\n")
    ...
    # Mostrar proyección 2D de la nube de puntos
    if show_pics:
        plt.figure(figsize=(8, 6))
        plt.scatter(points_3d[:, 0], points_3d[:, 2], s=1, c='b', alpha=0.5)
        plt.title('Proyección 2D de la Nube de Puntos (X-Z)')
        plt.xlabel('X (metros)')
        plt.ylabel('Z (metros)')
        plt.grid(True)
        plt.show()
