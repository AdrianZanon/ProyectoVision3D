...
def calibrate(showPics=True):
    calibrationDir = r'C:\Users\Inigo\Documents\GitHub\ProyectoVision3D\Camara\Fotos_Javier'
...
def etapa7_reconstruccion_3d(img_left_rect, img_right_rect, K, baseline):
    """
    Reconstrucción 3D a partir de imágenes rectificadas usando disparidad.

    Args:
        img_left_rect: Imagen izquierda rectificada (BGR o escala de grises)
        img_right_rect: Imagen derecha rectificada (BGR o escala de grises)
        K: Matriz intrínseca de la cámara
        baseline: Distancia entre cámaras (m)

    Returns:
        puntos_3d: Nube de puntos 3D reconstruida (Nx3)
    """
    # Convertir a escala de grises si no lo están
    if len(img_left_rect.shape) == 3:
        img_left_gray = cv.cvtColor(img_left_rect, cv.COLOR_BGR2GRAY)
    else:
        img_left_gray = img_left_rect.copy()

    if len(img_right_rect.shape) == 3:
        img_right_gray = cv.cvtColor(img_right_rect, cv.COLOR_BGR2GRAY)
    else:
        img_right_gray = img_right_rect.copy()

    # Crear objeto StereoSGBM para calcular disparidad
    window_size = 5
    min_disp = 0
    num_disp = 16 * 10  # Debe ser múltiplo de 16
    ...
    disparidad = stereo.compute(img_left_gray, img_right_gray).astype(np.float32) / 16.0

    # Q matriz de reproyección 4x4: la puedes construir con K y baseline
    # Asumiendo que la rectificación dejó las líneas horizontales, focal fx y baseline conocidos
    fx = K[0, 0]
    fy = K[1, 1]
    cx = K[0, 2]
    cy = K[1, 2]
    ...
    puntos_3d_hom = cv.reprojectImageTo3D(disparidad, Q)

    # Filtrar puntos con disparidad válida
    mask = disparidad > min_disp
    puntos_3d = puntos_3d_hom[mask]

    # Visualización con Open3D
    n_points = puntos_3d.shape[0]
    print(f"Número de puntos 3D reconstruidos: {n_points}")

    # Obtener colores para la nube (de la imagen izquierda rectificada)
    colores = img_left_rect[mask]

    # Crear nube de puntos Open3D
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(puntos_3d)
    pcd.colors = o3d.utility.Vector3dVector(colores.astype(np.float) / 255.0)
...
if __name__ == '__main__':
    ...
    F_rect, pts1_inliers, pts2_inliers = ransac_fundamental(pts1_rect, pts2_rect, iteraciones=1000)
    ...
    puntos_3d = etapa7_reconstruccion_3d(img1_rect, img2_rect, K, baseline)


    '''points_3d = generate_point_cloud(img_left_rect, img_right_rect, K, baseline, save_visualizations="report_visuals",
                                     show_pics=show_pics)
    if points_3d is None:
        print("Error: No se pudo generar la nube de puntos.")
        return None
