    ...
    def callback_raton(event, x, y, flags, param):
        if event == cv.EVENT_LBUTTONDOWN:
            if len(puntos) < num_puntos:
                puntos.append((x, y))
                cv.circle(imagen_mostrar, (x, y), 5, (0, 0, 255), -1)
...
def dibujar_lineas_epipolares_izq_a_der(img1, img2, F, puntos_izq):
    """
    Dibuja líneas epipolares en la imagen derecha a partir de puntos seleccionados en la izquierda.

    Args:
        img1: Imagen izquierda (para referencia)
        img2: Imagen derecha donde se dibujarán las líneas
        F: Matriz fundamental
        puntos_izq: Puntos seleccionados en la imagen izquierda
    """
    # Preparar imagen derecha para dibujar
    img_derecha = img2.copy() if len(img2.shape) == 3 else cv.cvtColor(img2, cv.COLOR_GRAY2BGR)

    # Dibujar líneas epipolares para cada punto izquierdo
    for pt in puntos_izq:
        # Calcular línea epipolar en imagen derecha: l2 = F * [x,y,1]^T
        ...
        h, w = img2.shape[:2]
        if abs(b) > 1e-6:  # Evitar división por cero
            x0, y0 = 0, int(-c / b)
            x1, y1 = w, int(-(a * w + c) / b)
            color = tuple(np.random.randint(0, 255, 3).tolist())
            cv.line(img_derecha, (x0, y0), (x1, y1), color, 2)

    # Reducir y mostrar imagen derecha con líneas
    img_derecha_resized = cv.resize(img_derecha, None, fx=0.5, fy=0.5)
    cv.imshow('Imagen Derecha - Lineas Epipolares (de puntos izquierdos)', img_derecha_resized)
    cv.waitKey(0)
    cv.destroyAllWindows()


def dibujar_lineas_epipolares_der_a_izq(img1, img2, F, puntos_der):
    """
    Dibuja líneas epipolares en la imagen izquierda a partir de puntos seleccionados en la derecha.

    Args:
        img1: Imagen izquierda donde se dibujarán las líneas
        img2: Imagen derecha (para referencia)
        F: Matriz fundamental
        puntos_der: Puntos seleccionados en la imagen derecha
    """
    # Preparar imagen izquierda para dibujar
    img_izquierda = img1.copy() if len(img1.shape) == 3 else cv.cvtColor(img1, cv.COLOR_GRAY2BGR)

    # Dibujar líneas epipolares para cada punto derecho
    for pt in puntos_der:
        # Calcular línea epipolar en imagen izquierda: l1 = F^T * [x,y,1]^T
        pt_hom = np.array([pt[0], pt[1], 1])
        l1 = F.T @ pt_hom
        a, b, c = l1

        # Dibujar línea en imagen izquierda
        h, w = img1.shape[:2]
        if abs(b) > 1e-6:  # Evitar división por cero
            x0, y0 = 0, int(-c / b)
            x1, y1 = w, int(-(a * w + c) / b)
            color = tuple(np.random.randint(0, 255, 3).tolist())
            cv.line(img_izquierda, (x0, y0), (x1, y1), color, 2)

    # Reducir y mostrar imagen izquierda con líneas
    img_izquierda_resized = cv.resize(img_izquierda, None, fx=0.5, fy=0.5)
    cv.imshow('Imagen Izquierda - Lineas Epipolares (de puntos derechos)', img_izquierda_resized)
    cv.waitKey(0)
    cv.destroyAllWindows()
...
if __name__ == '__main__':
    ...
    # Reducir todas las imágenes al 50% para visualización
    ...
    # Paso 1: Seleccionar puntos en imagen izquierda y calcular líneas en derecha
    print("\nSeleccione puntos en la imagen IZQUIERDA:")
    puntos_izq = capturar_puntos_manuales(
        img_izquierda,
        "Seleccione 6 puntos - Imagen Izquierda",
        6
    )
    ...
    # Paso 2: Seleccionar puntos en imagen derecha y calcular líneas en izquierda
    print("\nSeleccione puntos en la imagen DERECHA:")
    puntos_der = capturar_puntos_manuales(
        img_derecha,
        "Seleccione 5 puntos - Imagen Derecha",
        5
    )
    dibujar_lineas_epipolares_der_a_izq(
        img_izquierda,
        img_derecha,
        F,
        puntos_der
    ...
    centro = np.array([img_izquierda.shape[1] / 2, img_izquierda.shape[0] / 2])
    H1, H2 = rectificacion_estereoscopica_hartley(
        F,
        img_izquierda,
        img_derecha,
        inliers1,
    ...
    img_izquierda_rect, img_derecha_rect = aplicar_rectificacion_manual(
        img_izquierda,
        img_derecha,
        H1,
        H2
    ...
    # Reducir imágenes rectificadas al 50% para visualización
    ...
    # Verificar rectificación con puntos manuales
    print("\n--- DESPUÉS DE RECTIFICACIÓN ---")

    # Paso 1: Seleccionar puntos en imagen izquierda rectificada
    print("\nSeleccione puntos en la imagen IZQUIERDA RECTIFICADA:")
    puntos_izq_rect = capturar_puntos_manuales(
        img_izquierda_rect,
        "Seleccione puntos - Imagen Izquierda Rectificada",
        6
    )
    dibujar_lineas_epipolares_izq_a_der(
        img_izquierda_rect,
    ...
    # Paso 2: Seleccionar puntos en imagen derecha rectificada
    print("\nSeleccione puntos en la imagen DERECHA RECTIFICADA:")
    puntos_der_rect = capturar_puntos_manuales(
        img_derecha_rect,
        "Seleccione puntos - Imagen Derecha Rectificada",
        5
    )
    dibujar_lineas_epipolares_der_a_izq(
        img_izquierda_rect,
    ...
    # Dibujar líneas epipolares rectificadas automáticas para comparación
    print("\n--- VISUALIZACIÓN AUTOMÁTICA DE RECTIFICACIÓN ---")
